...
def wall():
    ...
    query = "SELECT CONCAT(users.first_name, ' ', users.last_name) AS full_name, messages.message, messages.id AS message_id, messages.created_at FROM users LEFT JOIN messages ON users.id = messages.user_id;"
    message_list = mysql.query_db(query)
    # print message_list

    query = "SELECT users.first_name, users.last_name, comments.comment, comments.created_at, comments.message_id FROM comments JOIN users ON comments.user_id = users.id"
    # data = {
